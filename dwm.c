...
clickstatusbar(const Arg *arg)
{
    ...
    while (stext[++offset] != '\0') {
        ...
        if (stext[offset] == '^' && !iscode) {
            iscode = 1;
            offset++;
            if (stext[offset] == 's') { // get the signal
                issignal = 1;
                signalindex = 0;
...
resizebarwin(Monitor *m) {
    ...
    if (showsystray && m == systraytomon(m))
        w -= system_w;
    XMoveResizeWindow(dpy, m->barwin, m->wx + sp, m->by + vp, w - 2 * sp - (m == systraytomon(m) && system_w ? systrayspadding : 0), bh);
}
